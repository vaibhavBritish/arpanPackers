    ...
    <section style={{ background: "#F8FAFC", padding: "5rem 0" }}>
      <div className="container-custom">
        <div className="reveal" style={{ textAlign: "center", marginBottom: "3rem" }}>
          ...
          <p className="section-subtitle" style={{ margin: "0 auto" }}>No hidden charges. What you see is what you pay. All plans include free survey.</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "1.5rem" }}>
          {plans.map((p, i) => (
            <div
              ...
              style={{
                background: p.highlight ? "#1D4ED8" : "#fff",
                color: p.highlight ? "#fff" : "#0F172A",
                borderRadius: 16, padding: "2rem",
                border: p.highlight ? "none" : "1px solid #E5E7EB",
                boxShadow: p.highlight ? "0 20px 60px rgba(29,78,216,0.25)" : "none",
                transition: "all 0.3s",
                transform: p.highlight ? "scale(1.04)" : "scale(1)",
              }}
            >
              {p.highlight && <div style={{ background: "#EA580C", color: "#fff", fontSize: "0.72rem", fontWeight: 700, padding: "4px 12px", borderRadius: 99, display: "inline-block", marginBottom: "0.75rem" }}>MOST POPULAR</div>}
              <div style={{ fontFamily: "Poppins, sans-serif", fontWeight: 700, fontSize: "1.15rem", marginBottom: "0.25rem" }}>{p.name}</div>
              <div style={{ fontFamily: "Poppins, sans-serif", fontWeight: 800, fontSize: "2rem", marginBottom: "0.25rem", color: p.highlight ? "#fff" : "#1D4ED8" }}>{p.price}</div>
              <div style={{ fontSize: "0.82rem", color: p.highlight ? "rgba(255,255,255,0.7)" : "#6b7280", marginBottom: "1.5rem" }}>{p.desc}</div>
              <ul style={{ listStyle: "none", padding: 0, margin: "0 0 1.5rem", display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                {p.features.map((f) => (
                  <li key={f} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "0.875rem", color: p.highlight ? "rgba(255,255,255,0.9)" : "#374151" }}>
                    <Check size={16} weight="bold" style={{ color: p.highlight ? "#4ade80" : "#16a34a", flexShrink: 0 }} />
                    {f}
                  </li>
                ))}
              </ul>
              <button
                ...
                style={{
                  width: "100%", border: p.highlight ? "2px solid rgba(255,255,255,0.4)" : "2px solid #1D4ED8",
                  background: p.highlight ? "rgba(255,255,255,0.15)" : "transparent",
                  color: p.highlight ? "#fff" : "#1D4ED8", borderRadius: 8, padding: "12px",
    ...
    <section className="bg-white py-10 md:py-16 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-10">
          <div className="text-sm text-blue-600 mb-2">❓ FAQ</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {faqs.map((f, i) => (
            <div
              key={f.q}
              className="bg-gray-50 border rounded-xl p-5 md:p-6 hover:shadow-sm transition"
            >
      ...
      <section style={{ background: "#fff", padding: "5rem 0" }}>
        <div className="container-custom">
          <div className="reveal" style={{ textAlign: "center", marginBottom: "3rem" }}>
            ...
            <h2 className="section-title">Complete Relocation Solutions</h2>
            <div className="divider" style={{ margin: "0.75rem auto 1rem" }} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "1.75rem" }}>
            {services.map((svc, i) => (
              <div
                ...
                style={{ background: "#fff", borderRadius: 16, border: "1px solid #E5E7EB", padding: "2rem", display: "flex", flexDirection: "column", gap: "1rem" }}
              >
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                  <div className={`icon-box ${svc.color}`}><svc.icon size={26} weight="duotone" /></div>
                  <span style={{ background: "#EFF6FF", color: "#1D4ED8", padding: "4px 12px", borderRadius: 99, fontSize: "0.78rem", fontWeight: 600 }}>{svc.price}</span>
                </div>
                <h3 style={{ fontFamily: "Poppins, sans-serif", fontWeight: 700, fontSize: "1.15rem", color: "#0F172A", margin: 0 }}>{svc.title}</h3>
                <p style={{ color: "#6b7280", fontSize: "0.875rem", lineHeight: 1.65, margin: 0 }}>{svc.desc}</p>
                <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {svc.features.map((f) => (
                    <li key={f} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "0.82rem", color: "#374151" }}>
                      <Check size={14} weight="bold" style={{ color: "#16a34a", flexShrink: 0 }} />{f}
                    </li>
                  ))}
                </ul>
                <button onClick={() => router.push("/get-quote")} style={{ marginTop: "auto", display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: "#1D4ED8", fontWeight: 600, fontSize: "0.875rem", cursor: "pointer", padding: 0 }}>
      ...
      <section style={{ background: "#1D4ED8", padding: "4rem 0", textAlign: "center" }}>
        <div className="container-custom">
          <div className="reveal">
            <h2 style={{ fontFamily: "Poppins, sans-serif", fontWeight: 800, fontSize: "clamp(1.5rem, 4vw, 2.2rem)", color: "#fff", margin: "0 0 1rem" }}>Can't Find What You Need?</h2>
            <p style={{ color: "#bfdbfe", marginBottom: "2rem" }}>We offer custom packages tailored to your specific requirements. Contact us for a personalized quote.</p>
            <div style={{ display: "flex", gap: "1rem", justifyContent: "center", flexWrap: "wrap" }}>
              ...
              <a href="tel:+919876543210" style={{ background: "rgba(255,255,255,0.15)", color: "#fff", border: "2px solid rgba(255,255,255,0.4)", borderRadius: 8, padding: "14px 28px", fontWeight: 600, textDecoration: "none" }}>
                📞 Call Us Now
              </a>
            </div>
          </div>
